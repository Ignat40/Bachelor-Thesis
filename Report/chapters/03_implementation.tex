...
Communication between the Unity client and the Django backend is established via HTTP API endpoints. The Unity application exchanges JSON data through Unity's \texttt{UnityWebRequest} class, implemented in a dedicated C\# script called \texttt{TherapyApiClient}.

These technologies were deliberately selected to achieve long-term scalability rather than only a prototype. Django's ORM means the underlying database engine could be swapped or scaled independently without rewriting application logic. Supabase, used here purely as a DBaaS, already exposes WebSocket support, serverless functions, and OAuth authentication, none of which are needed in the current prototype but would become essential for supporting concurrent therapists across institutions or for integrating with third-party identity providers. In addition, Unity communicates with Django exclusively through a REST API using JSON, which means the mobile client and the backend can evolve separately. MediaPipe's landmark detection pipeline runs on-device, which avoids server-side video processing costs that would otherwise become a bottleneck at scale. These technological choices suggest that the prototype could potentially absorb real-world load with targeted improvements rather than requiring a full redesign.
...
\begin{figure}[p]
    \centering
    \includegraphics[height=0.82\textheight,keepaspectratio]{figures/architecture.jpg}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.95\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/dashboard-exercise-library.png}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/exercise-creation.png}
        \caption{Exercise editor}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/exercise-assignment.png}
        \caption{Assignment dialog}
    \end{subfigure}
    \caption{Therapist dashboard screens for browsing, editing, and assigning exercises.}
    \label{fig:therapist-dashboard-exercise-flow}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Therapist dashboard overview showing patient status, recent activity, progress, and weekly sessions.}
    \label{fig:therapist-dashboard-overview}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{figures/class-diagram.png}
...
The \texttt{Exercise\_Orchestrator} functions around a queue. This queue is initialized with the JSON provided by the Django backend, which the \texttt{Exercise\_JSON\_Parser} component parses into a predefined level class. This class defines levels as a set of exercises, each with an exercise type and a set of options. Different option classes were wrapped in a parent generic option class as a way to generalize them. This allows the Exercise class to hold a single Options array instead of one for each exercise type. Using this level class, the \texttt{Exercise\_Orchestrator} is able to call high-level functions such as \texttt{Prompt\_User()} and \texttt{Show\_Next\_Word()} in the correct sequence. The queue is only advanced when \texttt{On\_User\_Ready()} is called, effectively making the flow wait for user input.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[height=0.32\textheight,keepaspectratio]{figures/unity-quest-screen.png}
        \caption{Explanation}
    \end{subfigure}
    \hfill
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[height=0.32\textheight,keepaspectratio]{figures/unity-camera-exercise.png}
        \caption{Mouth position explanation screen}
    \end{subfigure}
    \caption{Unity explanation screens shown before or during exercise practice.}
    \label{fig:unity-explanation-screens}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.38\textheight,keepaspectratio]{figures/unity-exercise-menu.png}
    \caption{Unity word carousel screen used to present and repeat the words in an exercise.}
    \label{fig:unity-word-carousel}
\end{figure}
...
The success of the system depends on the execution of the core functionalities defined in the system architecture design. The solutions proposed to address the challenges identified in \Cref{ch:problem-statement} were designed as functional prototypes and tested both practically and technologically, with data flow demonstrated to an active Speech-Language Pathologist. The evaluation session with the therapist showed positive feedback on the design and its alignment with the current needs. Below, we share the results and evaluation feedback.

\subsection{Recalling Proper Exercise Execution}
\label{subsec:recalling-proper-exercise-execution}

The explanation screen was developed iteratively in the Unity exercise flow. It shows the user text and audio explanations of how to perform the upcoming exercises. After doing so, a card showing how each relevant sound is performed is shown to the user. After the core exercises were established, supporting features such as audio explanations, visual guidance, word cards, and instructions were added. The reason for those aids was that the target group may not be confident readers, thus focusing on combining sound and visual cues. The explanation screen is shown automatically at the start of each level, using the sounds from the selected exercise. 

This solution was evaluated by a practicing SLP, who, during a prototype demonstration, approved the chosen approach. As said by the interviewed SLP, "Reminding kids how to do their exercises helps them do them correctly. They often forget how to pronounce certain sounds, so reminders help."
...
\begin{itemize}
    ...
    \item The SLP's positive statement that they would use such a system in their own practice supports the practical application of the concept. As said by them during an interview, "I would love to have access to such a product. It seems interesting, and it could really be helpful in my practice."
    \item The SLP appreciated the distinction between the therapist-facing dashboard and the user-focused mobile application, noting that therapists assigning targeted exercises while children completed them themselves is typical of realistic clinical workflows. As stated by them, "It's like me creating paper materials and assigning them as homework. It's a similar workflow."
    \item The SLP pointed to the mobile format as a specific practical advantage because mobile-based exercises eliminate the common issue of children forgetting or losing paper practice materials. They stated, "Kids lose their paper materials, so they often can't complete their home exercises. On the other hand, they always have their phone on hand, so having the exercises there removes the burden of having to carry extra materials".
\end{itemize}

\subsection{Limitations}
\label{subsec:limitations}

While results from initial testing were positive, the prototype has several issues that would need to be addressed before it could be considered production-ready. The Unity client was designed as a gamified application, which the prototype could not implement to the extent presented in the intended design. Other features also had to be scoped out since goals shifted during development to focus more on a proof of concept, Minimum Viable Product.

The face tracking feature, for example, mentioned in \Cref{sec:technologies-used}, was treated as exploratory rather than a core deliverable. The current prototype implementation shows that real-time face, mouth, and lip positions can be extracted and converted into measurable output that can later be used as a stricter method of evaluation.
...
Finally, these results should be interpreted within their appropriate scope. The evaluation was based on a single demonstration rather than repeated usage. The feedback supports the project's relevance, but it is not formal evidence of clinical effectiveness. Our evaluation does not equal clinical trials with real users and a large sample of therapists. Now that our confidence in the prototype is stronger, the future version should be clinically tested.